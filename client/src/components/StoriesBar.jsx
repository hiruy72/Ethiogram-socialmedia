...
import StoryViewr from './StoryViewr'

const StoriesBar = () => {
    const [stories, setStories] = useState([])
    const [showModal, setShowModal] = useState(false)
    const [viewStory, setViewStory] = useState(null) // corrected name
        ...
        <div className='w-screen sm:w-[calc(100vh-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4'>
            <div className='flex gap-4 pb-5'>
                ...
                {stories.map((story, index) => (
                    <div
                        onClick={() => setViewStory(story)} // corrected name
                        key={index}
                    ...
                    >
                        <img
                            ...
                            alt=""
                        />
                        <p className='absolute top-0 left-3 text-white/60 text-sm truncate max-w-24'>{story.content}</p>
            ...
            {/* view story modal */}
            {viewStory && (
                <StoryViewr viewStory={viewStory} setViewStory={setViewStory} />
            )}
        </div>
    )
}
